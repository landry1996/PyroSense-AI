...
CREATE INDEX idx_interventions_electrician ON interventions(assigned_electrician_id) WHERE assigned_electrician_id IS NOT NULL;
CREATE INDEX idx_interventions_scheduled ON interventions(scheduled_at) WHERE status IN ('PLANNED', 'ASSIGNED');
